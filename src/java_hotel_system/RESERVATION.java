...
public class RESERVATION {
    ...
    // in the reservation table we need to add two FOREIG N keys:
    // 1 for the client
    ...
    // 1 - When we ad a new reservation the room associated with it should be set to reserved = YES
    ...
    // 3 - check if the date in > the curren date
        ...
        try {            
            ...
            if (room.isRoomToReserved(room_number).equals("No")) {
                 ...
                 if (st.executeUpdate() > 0){                
                        room.setRoomToReservation(room_number, "Yes");
                        return true;
                }else{
                        return false;
                     }
     ...
     public boolean removeReservation(int reservation_id)
    {
        ...
        try {            
            ...
            if (st.executeUpdate() > 0){
                ...
                //room.setRoomToReservation(reservation_id, "No");
                room.setRoomToReservation(room_number, "No");
...
// create a function to display all rooms in jtable
    ...
    public int getRoomNumberfromreservation(int reservation_id)
    {    
        PreparedStatement ps;
        ResultSet rs;
